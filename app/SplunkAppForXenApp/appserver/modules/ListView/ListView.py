...
#
# ListView Python Results Handler - returns JSON to the underlying module
#
# required imports
...
# logging setup
import logging
logger = logging.getLogger('splunk.appserver.controllers.module.ListView')

class ListView(module.ModuleHandler):
    ...
    def generateResults(self, host_app, client_app, sid, earliest_time=None, latest_time=None, entity_name='results', postprocess=None):
        ...
        #
        if not sid:
            raise Exception('ListView.generateResults - sid not passed!')
            
        # get the search jobs
        try:
            job = splunk.search.getJob(sid, sessionKey=cherrypy.session['sessionKey'])
        except splunk.ResourceNotFound:
            return _('<p class="moduleException">[ListView module] Session Key Not Found</p>')
