#
# NotSideview Python Results Handler - returns JSON to the underlying module
#
# required imports
...
# logging setup
import logging
logger = logging.getLogger('splunk.appserver.controllers.module.NotSideview')

class NotSideview(module.ModuleHandler):
    ...
    def generateResults(self, host_app, client_app, sid, earliest_time=None, latest_time=None, entity_name='results', postprocess=None):
        ...
        #
        if not sid:
            raise Exception('NotSideview.generateResults - sid not passed!')
            
        # get the search jobs
        try:
            job = splunk.search.getJob(sid, sessionKey=cherrypy.session['sessionKey'])
        except splunk.ResourceNotFound:
            return _('<p class="moduleException">[NotSideview module] Session Key Not Found</p>')
